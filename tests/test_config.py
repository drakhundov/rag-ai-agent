...
def with_temp_conf(tmp_path, monkeypatch):
    ...
    with open(os.path.join(PROJ_DIR, "config.json")) as settings_f:
        (tmp_path / "config.json").write_text(settings_f.read())
...
def test_config_load(with_temp_conf):
    ...
    assert conf.prompt_templ_lst.system.input_variables is not None
    assert conf.prompt_templ_lst.system.template is not None
...
def test_path_resolution(with_temp_conf):
    ...
    assert conf.paths.router_url.startswith("http")
