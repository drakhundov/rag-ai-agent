...
class HeuristicRouter:
    ...
    def add_translation_step(self, method: TranslationMethod):
        ...
        if self.qlist is None:
            err.log_and_raise(logger, ValueError("`qlist` hasn't been initialized yet."))
        elif not isinstance(self.qlist, QueryList):
            err.log_and_raise(logger, ValueError("`qlist` must be an instance of QueryList."))
        elif not isinstance(method, TranslationMethod):
            err.log_and_raise(
                logger, ValueError("`method` must be an instance of TranslationMethod Enum.")
            )
    ...
    def run_route(self) -> QueryList:
        ...
        if not self.route_constructed:
            err.log_and_raise(
                logger, RuntimeError("Route hasn't been constructed yet. Call `route()` first.")
            )
